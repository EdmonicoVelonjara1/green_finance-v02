...
import { IStock } from "@/app/api/get-stock/route"

export default function OscillateursClient() {
  ...
  const rsiData = stockData.length > 0
    ? calculateRSI(
        stockData.map((d) => ({
          ...d,
          date: new Date(d.date),
          adjClose: d.close, // or use the correct value if available
          volume: (d as any).volume ?? 0, // fallback to 0 if not present
        })),
        Number.parseInt(rsiPeriod)
      )
    : []
